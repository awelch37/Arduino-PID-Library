...
// maybe use ifnan ( ) protection on T5, T2, T1, T0 later
...
PID3  myPID3(&T4, &outputHotBias, &targetT4, T4consKp, T4consKi, T4consKd, DIRECT);
...
void loop()
{
  ...
  setpointA =  (maxSF * ((maxA - minA) * (hotBias))) + minA; // hotBias is driven by PID3 and T4 (oxi-cat inlet), usually.
 ...
 // Serial.print (gapB) ;
  ...
  // for reference --- PID3 myPID3(&T4, &outputHotBias, &targetT4, T4consKp, T4consKi, T4consKd, DIRECT);
  ...
  */
  ...
  //Serial.println("**REFERENCE DATA ONLY FOR: PID3  myPID3(&T4, &outputHotBias, &targetT4, T4consKp, T4consKi, T4consKd, DIRECT)");
  ...
  myPID3.Compute();
  hotBias = outputHotBias;  // ??? not necessary as myPID3 modifies hotBias to reach T4target
  ...
  */
  ...
  // other values T0, T1, T2, T5 are not critical.  They are only observed.
  ...
  /* Serial.print ("NAN checker: y5, y4, y2, y1, y0 =   ");
    ...
    Serial.print (T0 );
  */
  //Serial.println("    line 785 executed ");
}
